...
from replay_buffer_transformer import ReplayBuffer
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        # self.g1 = GTrXL(state_dim,1,1, hidden_dims=800, batch_first=True)
        self.tanh = nn.Tanh()
...
class td3(object):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.writer = SummaryWriter(log_dir="/root/ros2_ws/src/td3/scripts/runs/distance_variable-" + datetime.now().strftime("%y-%m-%d-%H-%M"))
    ...
    def get_action(self, state_array):
        ...
        return self.actor(state_array).cpu().data.numpy()[0][0]
    ...
    def train(
        ...
        for it in range(iterations):
            ...
            state_array = torch.Tensor(batch_state_array).to(device)

            # Obtain the estimated action from the next state by using the actor-target
            next_action = self.actor_target(state_array)[:,0,:]
            ...
            if it % policy_freq == 0:
                ...
                actor_grad, _ = self.critic(state, self.actor(state_array)[:,0,:])
...
class GazeboEnv(Node):
    ...
    def __init__(self):
        ...
        self.object_msg = SetEntityState.Request()
        self.object_msg.state.name = "/td3_robot"
        self.object_msg.state.pose.position.x = 0.0
        self.object_msg.state.pose.position.y = 0.0
        self.object_msg.state.pose.position.z = 0.0

        # Set up the ROS publishers and subscribers
        self.vel_pub = self.create_publisher(Twist, "/cmd_vel", 1)
        self.set_proxy = self.create_client(SetEntityState,"/gazebo/set_entity_state")
    ...
    def reset(self):
        ...
        # future = self.set_proxy.call_async(object_state)


        # self.odom_x = object_state.state.pose.position.x
        # self.odom_y = object_state.state.pose.position.y
    ...
    def random_box(self):
        ...
        for i in range(1,9):
            ...
            while not box_ok:
                ...
                distance_to_robot = np.linalg.norm([x - 0.0, y - 0.0])
            ...
            future = self.set_proxy.call_async(box_state)
            print("done")
...
if __name__ == '__main__':
    ...
    expl_noise = 1.0  # Initial exploration noise starting value in range [expl_min ... 1]
    expl_decay_steps = (
    ...
    file_name = "td3_velodyne_distance_variable" + datetime.now().strftime("%y-%m-%d-%H-%M") # name of the file to store the policy
    save_model = True  # Weather to save the model or not
    load_model = False  # Weather to load a stored model
    random_near_obstacle = True  # To take random actions near obstacles or not

    # Create the network storage folders
    if not os.path.exists("/root/ros2_ws/src/td3/scripts/results"):
        os.makedirs("/root/ros2_ws/src/td3/scripts/results")
    if save_model and not os.path.exists("/root/ros2_ws/src/td3/scripts/pytorch_models"):
        os.makedirs("/root/ros2_ws/src/td3/scripts/pytorch_models")
    ...
    writer = SummaryWriter(log_dir="/root/ros2_ws/src/td3/scripts/runs/distance_variable-" + datetime.now().strftime("%y-%m-%d-%H-%M"))
    writer_reward = 0
    episode_reward = 0
            ...
            if timestep < max_timesteps:
                ...
                if done:
                    ...
                    if timesteps_since_eval >= eval_freq:
                        env.get_logger().info("Savingg")
                        timesteps_since_eval %= eval_freq
                        # evaluations.append(
                        #     evaluate(network=network, epoch=epoch, eval_episodes=eval_ep)
                        # )

                        network.save(file_name, directory="/root/ros2_ws/src/td3/scripts/pytorch_models")
                        # np.save("/root/ros2_ws/src/td3/scripts//results/%s" % (file_name), evaluations)
                        epoch += 1
                ...
                action = network.get_action(state_array)
                ...
                replay_buffer.add(state, action, reward, done_bool, next_state, state_array)

                # Update the counters
                state_array = np.roll(state_array,1,axis=0)
                state_array[0] = next_state
                state = next_state
                episode_timesteps += 1
                timestep += 1
