...
class td3(object):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.writer = SummaryWriter(log_dir="/root/ros2_ws/src/td3/scripts/runs/potential")
...
class GazeboEnv(Node):
    ...
    def __init__(self):
        ...
        self.prev_potential = 0.000
        self.episode_rewards = 0.00
        self.max_episode_rewards = -99999
        self.min_episode_rewards = 99999
    ...
    def get_reward(self, target, collision, action, min_laser, distance):
        if target:
            ...
            potential  =  1 + (self.episode_rewards - self.max_episode_rewards)/(self.max_episode_rewards - self.min_episode_rewards)
            if(potential == np.nan or potential == np.inf or potential == np.NINF):
                potential = 0
            potential_differnece = 0.9*(potential) - self.prev_potential
            self.prev_potential =  potential
            
            if(self.episode_rewards > self.max_episode_rewards):
                self.max_episode_rewards = self.episode_rewards

            if(self.episode_rewards < self.min_episode_rewards):
                self.min_episode_rewards = self.episode_rewards

            self.episode_rewards = 0
            self.prev_potential = 0

            return 100.0 + potential_differnece
        elif collision:
            env.get_logger().info("reward -100")
            self.episode_rewards+=-100
            ...
            costs =  (self.prev_distance - distance)
            self.prev_distance = distance
            self.episode_rewards += costs
            ...
            return costs + potential_differnece
...
if __name__ == '__main__':
    ...
    file_name = "td3_velodyne_potential"  # name of the file to store the policy
    ...
    writer = SummaryWriter(log_dir="/root/ros2_ws/src/td3/scripts/runs/potential")
    writer_reward = 0
    episode_reward = 0
