...
class td3(object):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.writer = SummaryWriter(log_dir="/home/anas/RL_research_workspace/src/td3/scripts/runs/bsrs")
...
class GazeboEnv(Node):
    ...
    def step(self, action):
        ...
        # The reward is now calculated in the main loop using BSRS
    ...
    def get_reward(self, target, collision, min_laser, distance):
        ...
        # The BSRS shaping is applied in the main training loop.
        if target:
...
if __name__ == '__main__':
    ...
    policy_freq = 2
    buffer_size = 1e6
    # BSRS hyperparameter eta (shape-scale)
    bsrs_eta = 1.0
    file_name = "td3_velodyne_bsrs"
    save_model = True
    load_model = False
    ...
    writer = SummaryWriter(log_dir="./runs/bsrs")
    writer_reward = 0
    episode_reward = 0
    ...
    try:
        while rclpy.ok():
            if timestep < max_timesteps:
                ...
                # --- BSRS Implementation Start ---
                # Calculate potential Phi for the current state s.
                # The paper suggests using the online critic and actor networks.
                # V(s) is approximated by Q(s, actor(s)).
                state_tensor = torch.Tensor(state.reshape(1, -1)).to(device)
                q1_s, q2_s = network.critic(state_tensor, network.actor(state_tensor))
                v_s = torch.min(q1_s, q2_s).cpu().data.numpy().flatten()[0]
                phi_s = bsrs_eta * v_s

                # Perform action in env
                next_state, reward, done, target = env.step(a_in)

                # Calculate potential Phi for the next state s'.
                next_state_tensor = torch.Tensor(next_state.reshape(1, -1)).to(device)
                q1_s_prime, q2_s_prime = network.critic(next_state_tensor, network.actor(next_state_tensor))
                v_s_prime = torch.min(q1_s_prime, q2_s_prime).cpu().data.numpy().flatten()[0]
                # The value of a terminal state is 0.
                v_s_prime = v_s_prime * (1 - int(done))
                phi_s_prime = bsrs_eta * v_s_prime

                # The final shaped reward according to the BSRS paper.
                shaped_reward = reward + (discount * phi_s_prime) - phi_s
                # --- BSRS Implementation End ---
