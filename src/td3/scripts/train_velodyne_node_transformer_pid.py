...
class td3(object):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.writer = SummaryWriter(log_dir="/root/ros2_ws/src/td3/scripts/runs/transformer_PID-" + datetime.now().strftime("%y-%m-%d-%H-%M"))
...
class GazeboEnv(Node):
    ...
    def __init__(self):
        ...
        self.pid =  PID(0.5,0.01,0.00001,0.1,1.0,(-2.5,2.5),(-0.7,0.7))
    ...
    def get_reward(self, target, collision, action, min_laser, distance):
        if target:
            ...
            costs = self.pid.update(0.0,abs(distance),time_difference)
            # print(costs)
            self.prev_time = time.time_ns()
...
if __name__ == '__main__':
    ...
    file_name = "td3_velodyne_transformer_PID"  + datetime.now().strftime("%y-%m-%d-%H-%M") # name of the file to store the policy
    ...
    writer = SummaryWriter(log_dir="/root/ros2_ws/src/td3/scripts/runs/transformer_PID" + datetime.now().strftime("%y-%m-%d-%H-%M"))
    writer_reward = 0
    episode_reward = 0
