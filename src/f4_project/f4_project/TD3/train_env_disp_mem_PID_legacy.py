...
class DroneGazeboEnv(gym.Env):
    def __init__(self):
        ...
        self.pid_distance = PID(Kp=0.1,Ki=0.000,Kd=0.01,Ku=2.0,sample_time=0.5,output_limits=(-2.0,2.0),integeral_limits=(-1.0,1.0))
        self.pid_forward = PID(Kp=0.01,Ki=0.00,Kd=0.01,Ku=1.0,sample_time=0.5,output_limits=(-1.0,1.0),integeral_limits=(-0.5,0.5))
    ...
    def reset(self,seed=None,options=None):
        ...
        state =  {"laser": self.laser_ranges  , "goal":self.goal_data}
        # if(len(state) == 720):
        self.previous_error = 0.0
    ...
    def step(self, action):
        ...
        vel_cmd = Twist()
        vel_cmd.linear.x = (action[0] + 1.5 )* 0.05
        ...
        if not self.done:
                ...
                # reward = (self.prev_distance - self.distance)*4.0 - (abs(action[1]) * 0.5)
                if(np.isnan(reward)):
                    reward = 0.0
