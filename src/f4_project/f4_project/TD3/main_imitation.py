...
from input_utils import KeyboardController # Import our new controller
...
if __name__ == "__main__":
	...
	parser.add_argument("--start_timesteps", default=5000, type=int)# Time steps for manual control phase
	...
	parser.add_argument("--run_name", default="imitation_run")       # Run name
	...
	keyboard_controller = KeyboardController(action_dim=2)
	...
	for t in range(150000):
		...
		if total_timesteps < args.start_timesteps:
			# Imitation Learning Phase: Use User Input
			action = keyboard_controller.get_action()
			# Ideally we might wanted to add some noise even here? Usually for imitation we want pure expert.
			# But for exploring, user is the expert.
			if total_timesteps % 100 == 0:
				print(f"Manual Control Phase: {total_timesteps}/{args.start_timesteps}")
		else:
			if total_timesteps == args.start_timesteps:
				print("Manual Phase Ended. RL Model taking over!")
