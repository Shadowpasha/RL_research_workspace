...
class Actor(nn.Module):
    def __init__(self, input_image_shape, goal_dim, action_dim, max_action):
        """
        Initializes the Actor network with separate processing paths
        for image features and goal coordinates, before concatenation.
        ...
        # --- CNN Block for Image Processing ---
        # The number of channels increases to learn progressively richer features
        self.conv1 = nn.Conv2d(in_channels=input_image_shape[0], out_channels=32, kernel_size=3, stride=1, padding=1)
        ...
        # This new network processes the goal coordinates independently.
        ...
        # The output size of the goal MLP
        goal_mlp_output_size = 64 

        # --- Fully Connected Layers for Fusion ---
        # The first linear layer now takes the flattened CNN output
        # combined with the new, embedded goal features.
        self.l1 = nn.Linear(flattened_cnn_size + goal_mlp_output_size, 800)
    ...
    def forward(self, state_image, goal_coords):
        """
        Forward pass through the Actor network, combining image features
        and embedded goal coordinates.
        ...
        """
        # Process image through the CNN blocks
        ...
        flattened_image_features = a.view(a.size(0), -1) 

        # Process goal coordinates through the new MLP
        embedded_goal_features = self.goal_mlp(goal_coords)
        
        # Concatenate flattened image features with embedded goal features
        combined_features = torch.cat([flattened_image_features, embedded_goal_features], dim=1) 
...
class Critic(nn.Module):
    def __init__(self, input_image_shape, goal_dim, action_dim):
        """
        Initializes the Critic network with separate processing paths
        for image features and goal coordinates, before concatenation.
        ...
        # The number of channels now progressively increases, mirroring the Actor's
        # architecture to avoid an information bottleneck.
        self.conv1 = nn.Conv2d(in_channels=input_image_shape[0], out_channels=32, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv3 = nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, stride=1, padding=1)
        ...
        # --- MLP for Goal Coordinates ---
        # This new network processes the goal coordinates independently, creating a
        # richer feature representation before it's combined with image features.
        self.goal_mlp = nn.Sequential(
            nn.Linear(goal_dim, 32),
            nn.ReLU(),
            nn.Linear(32, 64),
        ...
        # will now be: (flattened_cnn_output_size + embedded_goal_size + action_dim)
        combined_feature_dim = flattened_cnn_size + goal_mlp_output_size + action_dim
    ...
    def forward(self, state_image, goal_coords, action):
        ...
        # Flatten the CNN output features
        flattened_image_features = image_features.view(image_features.size(0), -1)

        # Process goal coordinates through the new MLP
        embedded_goal_features = self.goal_mlp(goal_coords)

        # Concatenate flattened image features, embedded goal features, and action
        # The action is now concatenated after the state features have been enriched.
        sa = torch.cat([flattened_image_features, embedded_goal_features, action], dim=1)
    ...
    def Q1(self, state_image, goal_coords, action):
        """
        Forward pass through the Q1 network to estimate a single Q-value.
        This is useful for calculating the loss.
        """
        # Process image through shared CNN layers
        image_features = F.relu(self.conv1(state_image))
        image_features = F.relu(self.conv2(image_features))
...
class TD3(object):
    ...
    def select_action(self, state):
        # Correctly reshape the state image based on its actual dimensions
        # This fixes a potential error from the previous implementation
        ...
        # Pass the correctly formatted state to the actor network
