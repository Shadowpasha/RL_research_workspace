...
class DroneGazeboEnv(gym.Env):
    def __init__(self):
        ...
        self.sub_disparity  = self.node.create_subscription(LaserScan,"/scan",self.get_laser, qos_profile_laser)

        # self.states_sub = self.node.create_subscription(ModelStates,"/model_states",self.get_model_states, 10)
        ...
        self.action_space = spaces.Box(np.array([-1,-1,-1]),np.array([1,1,1]),(3,),dtype= np.float64) 
        self.spaces = {
                'laser': spaces.Box(0.0, 5.5, shape=(3,20), dtype= np.float64),
                'goal': spaces.Box(-20.0,20.0,shape=(7,), dtype= np.float64)
                    }
        ...
        self.et.start()
        self.close = False
        self.laser_ranges = np.zeros((3,20))
    ...
    def reset(self,seed=None,options=None):
        ...
        state =  {"laser": data , "goal":np.array([(self.goal[0] - self.pose.pose.position.x) / self.goal[0] ,(self.goal[1] - self.pose.pose.position.y)/ self.goal[1], self.goal_heading/3.14 ,self.vel.twist.linear.x/0.4, self.vel.twist.linear.y/0.2, self.vel.twist.linear.z/0.05,self.distance/self.original_distance])}
        # if(len(state) == 720):
    ...
    def step(self, action):
        ...
        vel_cmd = Twist()
        vel_cmd.linear.x = (action[0] + 1.2 )* 0.1
        vel_cmd.linear.y = (action[1] * 0.1)
        vel_cmd.linear.z = (action[2] * 0.05)
        ...
        state =  {"laser": data , "goal": np.array([(self.goal[0] - self.pose.pose.position.x) / self.goal[0] ,(self.goal[1] - self.pose.pose.position.y)/ self.goal[1], goal_heading/3.14 ,self.vel.twist.linear.x/0.4, self.vel.twist.linear.y/0.2, self.vel.twist.linear.z/0.05,self.distance/self.original_distance])}
        ...
        if not self.done:
                reward = (self.prev_distance - self.distance)*4.0 - (abs(action[1]) * 1.2)
                if(np.isnan(reward)):
                    reward = 0.0
    ...
    def check_pos(self,x,y):
        ...
        for model in self.tree_locations:
            if( model.position.x + 1.7 > x > model.position.x - 1.7 and model.position.y + 1.7 > y > model.position.y - 1.7):
                pos_ok = False
    ...
    def randomize_trees(self):
        ...
        for i in range(1,15):
            ...
            tree_y = 0.0
            while not tree_ok:
                tree_x = random.uniform(-1.0,10.0)
                tree_y = random.uniform(-5.0,5.0)
        ...
        # try:
        #     self.reset_proxy.call(reset_msg)
        # except (rospy.ServiceException) as e:
